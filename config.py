...
RUN = dict(
    ...
    max_repairs=3,
    gen_wallclock_s=1800,
    spend_cap_usd=100.0,
    spend_accounting="api_equivalent_estimate",
    retire_after=5,
    systemic_halt_after=3,
    llm="codex_oauth",
    planner_llm="gpt-6-astra",
    subagent_llm="gpt-6-astra",
    curator_llm="gpt-6-astra",
    adapter_llm="gpt-6-astra",
    require_traces=True,
    max_call_seconds=600,
    max_call_reservation_usd=10.0,
    run_wallclock_s=10800,
    wandb_mode="online",
...
def active_config():
    ...
    defaults = dict(seed=1729, dtype="float32", batch_size=16,
        benchmark_protocol="whole_step_inductor_v1",
        step_backend="inductor",
        functional_discovery=True,
        subagent_models=[], planner_effort="high",subagent_effort="high",curator_effort="medium",adapter_effort="medium",
