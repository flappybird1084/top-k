...
def _(mo,snapshot,refresh):
    ...
    _rows=[{'kernel':r['lineage_id'],'candidate':r['id'],'generation':r['generation'],
            ...
            ('val_loss' if _recipe else 'step_ms'):r.get('val_loss' if _recipe else 'step_time_ms')} for r in (_state.get('architecture',{}).get('candidates',[]) if _recipe else _state.get('candidates',[]))]
