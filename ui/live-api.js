'use strict';
(() => {
 const PUBLIC=!!window.TOPK_PUBLIC_PAGE;
 ...
 async function refreshIdentity(){
  try{
   ...
   if(!config.enabled){lock(PUBLIC?'':'Access is protected. GitHub sign-in is awaiting setup.');return;}
   ...
   lock(PUBLIC?'':'Sign in with GitHub to use Top-Kernel.');
  }catch{config=null;lock(PUBLIC?'':'Access is protected. The sign-in service is unavailable.');}
 }
 ...
 document.addEventListener('DOMContentLoaded',()=>{
  ...
  paint(PUBLIC?'':'Checking access…');refreshIdentity();
 });
 window.fetch=async(input,options={})=>{
  ...
  if(response.status===401){localStorage.removeItem(key);user=null;lock(PUBLIC?'':'Your session expired. Sign in again.');}
  return response;
 };
 window.topkAuth={
  ...
  beginSignIn,
  onChange:cb=>window.addEventListener('topk-auth-changed',cb),
 };
})();
