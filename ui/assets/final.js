...
async function load(){
 ...
 let pair;
 {
  const fetchRun=async id=>{const r=await fetch('/api/runs/'+encodeURIComponent(id),{signal:AbortSignal.timeout(10000)});if(!r.ok)throw Error('Run unavailable');const s=await r.json();if(s.id!==id)throw Error('Run mismatch');return s};
  const id=params.get('run');if(!id)throw Error('No measurements recorded for this run.');
  const s=await fetchRun(id);pair={[s.mode==='recipe'?'architecture':'kernel']:s};
  await Promise.allSettled(['architecture','kernel'].map(async side=>{const related=s.related_runs?.[side];if(related&&related!==id)pair[side]=await fetchRun(related)}));
 }
 ...
 document.querySelector('#result-status').textContent='Run results';
 }catch(e){document.querySelector('#architecture-result').innerHTML=architecture(null);document.querySelector('#kernel-result').innerHTML=kernel(null);const n=document.querySelector('#result-error');n.hidden=false;n.textContent=e.message;document.querySelector('#result-status').textContent='Results unavailable';}
}
load();
