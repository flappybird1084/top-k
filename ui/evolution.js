...
function architectureRender(snapshot){
 ...
 panels.innerHTML=gens.map((g,gi)=>{
  ...
  return `<details class="gen-panel" data-key="g${g.n}"${isOpen?' open':''}><summary>${genLabel(g)} — ${g.cands.length} candidate(s), ${acc} improved${g.budget?` · ${g.budget}s budget`:''}${tok}</summary>`+g.cands.map(r=>{
   ...
   return `<details class="cand-row" data-key="c${r.id}"${open.has('c'+r.id)?' open':''}><summary><span class="pill ${cls}">${label}</span><span class="cand-head">${escapeHTML(head)}</span></summary><div class="cand-body"><p>${escapeHTML(r.strategy||'')}</p><dl>${r.model_params?`<div><dt>Parameters</dt><dd>${(r.model_params/1e6).toFixed(1)}M</dd></div>`:''}${Number.isFinite(r.val_loss)?`<div><dt>Val loss</dt><dd>${r.val_loss.toFixed(4)} @${r.train_secs}s</dd></div>`:''}${Number.isFinite(r.repairs_used)?`<div><dt>Repairs used</dt><dd>${r.repairs_used}</dd></div>`:''}${r.tokens_in||r.tokens_out?`<div><dt>Tokens</dt><dd>${Number(r.tokens_in||0).toLocaleString()} in / ${Number(r.tokens_out||0).toLocaleString()} out</dd></div>`:''}${r.model_name?`<div><dt>Agent model</dt><dd>${escapeHTML(r.model_name)}</dd></div>`:''}${r.parent_id?`<div><dt>Parent</dt><dd>#${r.parent_id}</dd></div>`:''}${r.failure_note?`<div><dt>Failure</dt><dd>${escapeHTML(String(r.failure_note).slice(0,600))}</dd></div>`:''}</dl></div></details>`;
  }).join('')+`</details>`;
 }).join('');
}
