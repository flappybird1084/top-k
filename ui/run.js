...
function replaySnapshot(snapshot){
 if(!snapshot.preview_notice)return snapshot;
 const source=snapshot.candidates||[],seeds=source.filter(r=>r.generation===0),jobs=source.filter(r=>r.generation!==0);
 const completed=Math.min(jobs.length,Math.floor((Date.now()-replayStarted)/6000));
 const job=jobs[completed];
 return {...snapshot,status:job?'running':'complete',candidates:[...seeds,...jobs.slice(0,completed)],
 preview_notice:'Recorded run replay · '+completed+' / '+jobs.length+' evaluations shown',
 active_evaluations:job?[{id:job.id,kernel:job.lineage_id,strategy:job.strategy,started_at:(replayStarted+completed*6000)/1000,stage:'Replaying recorded evaluation'}]:[]};
}
function render(snapshot){
 ...
 q('#candidates').innerHTML=rows.slice(-12).reverse().map(r=>`<tr><td>${escapeHTML(r.lineage_id)}<small>${escapeHTML(r.id)}</small></td><td><details><summary>${escapeHTML((r.strategy||"Candidate").split(". ")[0])}</summary><p>${escapeHTML(r.strategy)}</p></details></td><td class="${r.accepted?'accepted':'rejected'}">${r.accepted?'✓ Verified':'Gate '+escapeHTML(r.gate_reached)+' · rejected'}</td><td>${Number.isFinite(r.step_time_ms)?r.step_time_ms.toFixed(2)+' ms':'—'}</td></tr>`).join('');
