...
GitHub identity is handled by the existing Python gateway, not by the static Sites host or its ChatGPT sign-in system. The website address stays unchanged. This integration is prepared but requires OAuth registration and a stable gateway deployment at `https://top-k.andredlcruz.com` before publishing the new client.
...
3. Set these values only in the gateway's private environment:
   ...
   - `GITHUB_AUTH_ORIGIN=https://top-k.andredlcruz.com`
   - `GITHUB_AUTH_DB`: durable private SQLite path (defaults to `~/.local/state/kernel-evolution/github-auth.sqlite`)
4. The `backend` origin in `ui/live-api.js` is configured as `https://top-k.andredlcruz.com`. Copy the updated client, `github-auth.css`, and `front.js` into the existing Sites checkout's `dist` directory. Publish the same Sites project; do not create another site or change its slug.
