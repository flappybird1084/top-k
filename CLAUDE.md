...
Implemented per **Kernel Evolution — Design Spec v3**; the spec still governs the invariants below. Layout: `search.py` (entry), `web.py` (barebones frontend — placeholder, the real one is being built externally and will replace it), `config.py` (DEV/RUN profiles), `kernelevo/` (harness: ops registry, archive, gates + workers, LLM providers, loop, repo/adapter pipeline), `adapters/` (JEPA + LM demo models), `notebooks/` (molab launcher + sqlite viewer). Secrets go in `.env` (committed with empty values — never commit filled keys).
...
- **The verifier ladder is external and deterministic** (gates ordered cheapest-first):
  ...
  4. Faster in-model (10 warmup + 30 timed steps). This gate produces the objective: `step_time_ms`, `samples_per_s`, `mfu`.
...
- **LLM provider interface** is a `Protocol` (`complete`, `usage_usd`) with per-role config (`planner_llm`, `subagent_llm`, `curator_llm`); Anthropic/OpenAI/W&B-Inference/Stub implementations. Web search is the provider's native search tool, exposed to the planner only.
...
- Web frontend: `python web.py` → http://127.0.0.1:8420. Jobs run serially in a worker thread (job state under `jobs/<id>/`): "local" shells out to `search.py`; "molab remote" (`kernelevo/molab.py`) drives the notebook's marimo kernel HTTP API (`GET /api/sessions`, `POST /api/kernel/execute` with Bearer token + `Marimo-Session-Id`, SSE response) — it uploads the project as a base64 tarball, installs deps, launches `search.py` detached with log/exit files, and polls the log back. Connection details come from the notebook's "Pair with agent" prompt (URL + token; the token shown on screen is masked — only the copied prompt has the real one). Session ids go stale on browser reconnect, so the session is re-resolved every call and the notebook tab must stay open.
...
- `--lineage=<name>` restricts a run to one lineage; `--llm provider[:model]` overrides all three roles.
...
- No test framework is set up; verification is the stub-mode run plus targeted `python - <<EOF` scripts against modules (that is how the code was validated).
