...
The landing-page **Both** option queues two independent searches on the same repository and dataset: architecture first, then kernels through the serial GPU worker. Both job IDs are saved in `related_runs` so dashboard tabs open the corresponding run automatically. Kernel search uses the original model, not an unverified architecture handoff. Repeated data submission does not duplicate either job.

Repository intake now runs a bounded, read-only Codex OAuth discovery agent with live web search before requesting data. It inspects training/config/data-source evidence, offers up to four cited dataset choices, and waits for the user's confirmation before queueing GPU work. Search failure falls back to a manual data link; it never silently substitutes a dataset. Existing active runs retain their chosen data. Restart the UI server to load backend changes after active dispatches finish.


The **Diagram page** link in the top-right of each performance view opens `/assets/search.html?run=<job-id>`. The floating workspace centers architecture and kernel orchestrators, with separate sub-agent blocks in outward generation columns. Baselines remain visible. Blocks use recorded parent links, model sprites, status colors, and an animated working ring; long IDs truncate and selecting a block opens details. `?demo=1` is explicitly illustrative playback. Live provider icons use candidate metadata or the configured sub-agent provider; unknown models remain neutral.

Started evaluations are retained from the event log until matched to the archive. A lost notebook connection keeps unarchived jobs visible with unknown status instead of pretending they are still running. Older recipe events lack stable candidate IDs, so their generic budget labels may remain marked awaiting sync after corresponding measurements arrive. This requires engine event IDs to fully reconcile.

Parallel architecture/kernel execution requires two distinct GPUs. The current Both intake remains serial; the walkthrough assigned the queued kernel job to a second notebook and launched its dispatcher separately. A file lock and persisted dispatch result prevent that job's later queue entry from launching it a second time. Interrupted dispatches require remote inspection before retrying. Notebook tokens, job state, and account credentials are local and excluded from Git.
